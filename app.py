...
def detect_diatoms(preprocessed_image, confidence_threshold=0.5):
    ...
    for contour in contours:
        ...
        if area > 100 and area < 5000:  # Adjust these thresholds
            ...
            detections.append(
                {"bbox": [x, y, w, h], "confidence": 1.0, "area": area}  # Placeholder
            )
    ...
    if uploaded_file is not None:
        try:
            ...
            if len(image_array.shape) == 3 and image_array.shape[2] > 3:
                st.error(
                    "This appears to be a special format image, not a standard microscopy image."
                )
                return
            ...
            if image_array.shape[0] < 100 or image_array.shape[1] < 100:
                st.warning(
                    "Image dimensions are unusually small for microscopy. Results may not be accurate."
                )
